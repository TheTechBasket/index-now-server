...
import { api, type KeyVerifyResult, type Site } from '@/lib/api'
...
export function Dashboard() {
  ...
  async function runBulk(action: 'submit' | 'verify' | 'sync', sites: Site[]) {
    ...
    toast.success(`${verb} ${ok} site${ok !== 1 ? 's' : ''}${fail ? `, ${fail} failed` : ''}`)
    clearSelection()
    reload()
  }
      ...
      {selectedSites.length > 0 && (
        <div className="animate-enter mb-4 flex flex-wrap items-center gap-2 rounded-md border bg-muted/40 px-3 py-1.5">
          ...
          <Button size="sm" variant="outline" onClick={() => runBulk('submit', selectedSites)} disabled={bulkBusy !== null}>
            <Send aria-hidden /> {bulkBusy === 'submit' ? 'Submitting…' : 'Submit'}
          </Button>
          <Button size="sm" variant="outline" onClick={() => runBulk('sync', selectedSites)} disabled={bulkBusy !== null}>
            <RotateCw className={`size-3.5 ${bulkBusy === 'sync' ? 'animate-spin' : ''}`} aria-hidden /> {bulkBusy === 'sync' ? 'Syncing…' : 'Sync'}
          </Button>
          <Button size="sm" variant="outline" onClick={() => runBulk('verify', selectedSites)} disabled={bulkBusy !== null}>
            <ShieldCheck aria-hidden /> {bulkBusy === 'verify' ? 'Verifying…' : 'Verify'}
          </Button>
          <Button
            size="sm"
            variant="destructive"
                ...
                <CardFooter className="flex items-center justify-between border-t bg-muted/20 px-4 py-2">
                  <div className="flex items-center gap-1">
                    <Button
                      ...
                      disabled={busy !== undefined}
                      title={busy === 'submit' ? 'Submitting…' : 'Submit pending URLs'}
                      aria-label="Submit"
                  ...
                  <TableRow key={site.id} data-selected={selected.has(site.id)}>
                    ...
                    <TableCell>
                      <a href={`/site/${site.id}`} className="block">
                        <div className="font-medium leading-tight">{site.name}</div>
                    ...
                    <TableCell className="text-right tabular-nums">{site.urlCounts.total}</TableCell>
                    ...
                    <TableCell className="text-xs">
                      {site.lastSubmission ? (
                        <div className="flex items-center gap-1.5">
                          <span>{relTime(site.lastSubmission.createdAt)}</span>
                          <Badge
                            variant={submissionStatusVariant[site.lastSubmission.status]}
                            className={lastStatus === 'error' ? 'animate-attention' : ''}
                          >
  ...
  return (
    <span
      className={
        ...
        (tone || highlight ? 'border-current/20 bg-current/5' : '')
      }
    >
